(()=>{
  ...
  function rebuildPointOptions(preferred){
    ...
    else if(arr.length)sel.value=arr[0][0];
    sel.dataset.vibSelected=sel.value;
  }

  function configureDisplay(){
    const display=document.getElementById('vibrationDisplayMode');
    if(display){display.innerHTML='<option value="selected">จุดที่เลือก</option>';display.value='selected';display.onchange=null;display.removeAttribute('onchange')}
  }
  ...
  function configureSelector(){
    ...
    sel.addEventListener('change',function(e){
      this.dataset.vibSelected=this.value;
  ...
  function draw(preferred,rebuild=true){
    if(rebuild)rebuildPointOptions(preferred);
    configureDisplay();configureSelector();configureInputs();ensureReference();
  ...
  if(typeof original==='function'){
    const wrapped=function(...args){
      const selected=document.getElementById('vibrationPoint')?.dataset.vibSelected||document.getElementById('vibrationPoint')?.value||'';
      const r=original.apply(this,args);
      setTimeout(()=>draw(selected,true),0);
      return r;
    };
    window.renderConditionTrends=wrapped;try{renderConditionTrends=wrapped}catch(e){}
  }
  document.addEventListener('change',e=>{
    if(['dashChecklist','dashMachine','dashOperator','dashDateFrom','dashDateTo'].includes(e.target?.id)){
      const selected=document.getElementById('vibrationPoint')?.dataset.vibSelected||'';
      setTimeout(()=>draw(selected,true),20);
    }
  });
