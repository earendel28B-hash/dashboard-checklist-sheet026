(()=>{
  ...
  function configureDisplay(){
    ...
    if(display){display.innerHTML='<option value="selected">จุดที่เลือก</option>';display.value='selected'}
  }
  ...
  function configureInputs(){
    ...
    document.querySelectorAll('tr.item-row').forEach(row=>{
      ...
      const input=row.querySelector(`input[name="vibration_${n}"]`);
      const na=row.querySelector('.vibration-na');
      if(isPulleyPoint(item)){
        ...
        const td=input.closest('td');if(td){td.className='vibration-na';td.innerHTML='<span title="วัด Vibration เฉพาะ Gearbox และ Pulley">—</span>'}
      }
    });
  }
  ...
  function latestByDate(kind,n){
    ...
    records().forEach(r=>{
      ...
      const t=templateFor(r),item=t?.items?.find(i=>Number(i.no)===n);if(!item)return;
  ...
  function draw(){
    rebuildPointOptions();configureDisplay();configureInputs();ensureReference();
    ...
    const byDate=latestByDate(kind,n),dates=[...byDate.keys()].sort();
    const inputs=kind==='gearbox'?dates.map(d=>byDate.get(d).input):[];
    const outputs=kind==='gearbox'?dates.map(d=>byDate.get(d).output):[];
    const singles=kind==='pulley'?dates.map(d=>byDate.get(d).single):[];
    drawCanvas(canvas,dates,kind,inputs,outputs,singles);
    updateSummary(kind,inputs,outputs,singles);
    const note=canvas.closest('.condition-card')?.querySelector('.condition-note');
    if(note)note.textContent=kind==='gearbox'?'Trend Gearbox — Input และ Output จากข้อมูลที่บันทึกใน Checklist':'Trend Pulley — Vibration ที่จุด Bearing ตามรายการที่เลือก';
  }

  function drawCanvas(canvas,dates,kind,inputs,outputs,singles){
    const ctx=canvas.getContext('2d'),rect=canvas.getBoundingClientRect(),q=window.devicePixelRatio||1;
    const w=Math.max(360,rect.width||700),h=Math.max(285,rect.height||285);
    canvas.width=w*q;canvas.height=h*q;ctx.setTransform(q,0,0,q,0,0);ctx.clearRect(0,0,w,h);
    const series=kind==='gearbox'?[{name:'Input',vals:inputs,color:'#236c93'},{name:'Output',vals:outputs,color:'#d47a23'}]:[{name:'Vibration',vals:singles,color:'#236c93'}];
    ...
    const p={l:55,r:24,t:35,b:48},plotW=w-p.l-p.r,plotH=h-p.t-p.b;
    const max=Math.max(8,Math.ceil((Math.max(...all,LIMITS.alert)+.5)*2)/2),min=0;
    const y=v=>p.t+plotH*(1-(v-min)/(max-min));
    const x=i=>dates.length>1?p.l+plotW*i/(dates.length-1):p.l+plotW/2;

    // screening zones
    const bands=[[0,LIMITS.normal,'rgba(25,135,84,.07)'],[LIMITS.normal,LIMITS.watch,'rgba(255,193,7,.08)'],[LIMITS.watch,LIMITS.alert,'rgba(255,126,0,.07)'],[LIMITS.alert,max,'rgba(220,53,69,.06)']];
    bands.forEach(([a,b,c])=>{ctx.fillStyle=c;ctx.fillRect(p.l,y(b),plotW,y(a)-y(b))});
    ctx.font='10px system-ui,sans-serif';
    for(let i=0;i<=4;i++){const v=max-(max-min)*i/4,py=y(v);ctx.strokeStyle='#d7e0e6';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(p.l,py);ctx.lineTo(w-p.r,py);ctx.stroke();ctx.fillStyle='#667784';ctx.textAlign='right';ctx.fillText(v.toFixed(1),p.l-7,py+3)}
    ...
    series.forEach(s=>{
      ctx.strokeStyle=s.color;ctx.fillStyle=s.color;ctx.lineWidth=2.4;ctx.beginPath();let started=false;
      s.vals.forEach((v,i)=>{if(!Number.isFinite(v))return;const px=x(i),py=y(v);if(!started){ctx.moveTo(px,py);started=true}else ctx.lineTo(px,py)});ctx.stroke();
      s.vals.forEach((v,i)=>{if(!Number.isFinite(v))return;const px=x(i),py=y(v);ctx.beginPath();ctx.arc(px,py,4,0,Math.PI*2);ctx.fill();ctx.font='700 10px system-ui,sans-serif';ctx.textAlign='center';ctx.fillText(v.toFixed(2),px,Math.max(p.t+9,py-9))});
    });
  ...
  function updateSummary(kind,inputs,outputs,singles){
    const anchor=document.getElementById('vibChecked'),summary=anchor?.closest('.condition-summary');if(!summary)return;
    const valid=a=>a.filter(Number.isFinite);
    if(kind==='gearbox'){
      const iv=valid(inputs),ov=valid(outputs),li=[...inputs].reverse().find(Number.isFinite),lo=[...outputs].reverse().find(Number.isFinite),all=[...iv,...ov];
      summary.innerHTML=`<span>ล่าสุด Input: <b>${li==null?'-':li.toFixed(2)+' mm/s'}</b></span><span>ล่าสุด Output: <b>${lo==null?'-':lo.toFixed(2)+' mm/s'}</b></span><span>สูงสุด: <b>${all.length?Math.max(...all).toFixed(2)+' mm/s':'-'}</b></span><span>สถานะ: <b>${status(all.length?Math.max(li??-Infinity,lo??-Infinity):null)}</b></span><span>จำนวนวันที่มีข้อมูล: <b id="vibChecked">${new Set([...inputs.map((v,i)=>Number.isFinite(v)?i:null),...outputs.map((v,i)=>Number.isFinite(v)?i:null)].filter(v=>v!=null)).size}</b></span>`;
    }else{
      const sv=valid(singles),latest=[...singles].reverse().find(Number.isFinite);
      summary.innerHTML=`<span>ล่าสุด: <b>${latest==null?'-':latest.toFixed(2)+' mm/s'}</b></span><span>ต่ำสุด: <b>${sv.length?Math.min(...sv).toFixed(2)+' mm/s':'-'}</b></span><span>สูงสุด: <b>${sv.length?Math.max(...sv).toFixed(2)+' mm/s':'-'}</b></span><span>สถานะ: <b>${status(latest)}</b></span><span>จำนวนครั้ง: <b id="vibChecked">${sv.length}</b></span>`;
    }
  }

  function ensureReference(){
    const canvas=document.getElementById('vibrationTrendChart'),card=canvas?.closest('.condition-card');if(!card)return;
    let note=card.querySelector('.vib-standard-reference');if(!note){note=document.createElement('div');note.className='vib-standard-reference';note.innerHTML='<b>Screening reference:</b> ISO 20816-3 / ISO 20816-9. Normal &lt;2.3, Watch 2.3–&lt;4.5, Alert 4.5–&lt;7.1, Danger ≥7.1 mm/s RMS <span>(ใช้เป็นเกณฑ์คัดกรองเบื้องต้น)</span>';canvas.closest('.chart-wrap')?.before(note)}
  }
  ...
  if(typeof original==='function'){
    const wrapped=function(...args){const r=original.apply(this,args);setTimeout(draw,0);return r};
    window.renderConditionTrends=wrapped;try{renderConditionTrends=wrapped}catch(e){}
  }
  document.addEventListener('change',e=>{if(['vibrationPoint','vibrationDisplayMode','dashChecklist','dashMachine','dashOperator','dashDateFrom','dashDateTo'].includes(e.target?.id))setTimeout(draw,20)});
